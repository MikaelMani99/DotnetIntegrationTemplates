...
namespace Utkeyrslukerfi.API.Services.Helpers
{
    ...
    {
        public static async Task<T> DeserializeJsonToObject<T>(this HttpResponseMessage response, string encapsulatedDataName = "data", bool flatten = false)
        {
            var json = await response.Content.ReadAsStringAsync();
            var jsonObject = JObject.Parse(json);
            var data = jsonObject[encapsulatedDataName];
            if (flatten)
            {
        ...
        public static async Task<IEnumerable<T>> DeserializeJsonToList<T>(this HttpResponseMessage response, string encapsulatedDataName = "data", bool flatten = false)
        {
            var json = await response.Content.ReadAsStringAsync();
            var jsonObject = JObject.Parse(json);
            var data = jsonObject[encapsulatedDataName];
